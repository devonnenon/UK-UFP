...
plotdata <- do.call(rbind,lapply(names(dlist), function(site){
  data <- dlist[[site]]
  data$site <- site
...
ggplot(data = plotdata, aes(x = date, y = value))+ 
  geom_vline(data = plotdata[which(plotdata$area == "wmid"),], 
...
lagsplotdf <- do.call(rbind, lapply(locationsplot, function(location){
  outcomes_result <- do.call(rbind, lapply(outcomes, function(outcome){
    totalest <- extlaglist[[location]][[outcome]][["estperc"]]
    lagsest <- extlaglist[[location]][[outcome]][["tabperc"]]
    df <- data.frame(
...
lagsplotdf %>%
 ...
 # scale_y_continuous(limits = c(-6, 6))+
  scale_x_continuous(breaks = c(-2, -1, 0, 1, 2, 3, 4, 5), 
                     labels = c("Net effect", "", 0, 1, 2, 3, 4, 5))+
  theme(strip.background = element_blank(), strip.placement = "outside") +
  facet_grid(location ~ outcome, 
             switch = "y", 
             #scales = "free_y", 
...
intplotdf <- do.call(rbind, lapply(intlocs, function(location){
  outcomes_result <- do.call(rbind, lapply(outcomes, function(outcome){
    est <- intlist[[location]][[outcome]][["estperc"]]
    df <- data.frame(
      est = est[,1],
      est_cilow = est[,2],
      est_cihigh = est[,3],
...
intplotdf %>%
  ...
  facet_grid(location ~ outcome, 
             switch = "y", 
             #scales = "free_y", 
             labeller = labeller(outcome = outcomelabs))
